...
`review_branch`, `check_encoding`, `fix_lint` (applies the upstream lint
fixers in-memory and returns the fixed text for the agent to write).
...
`generate_gfx_entry`, `generate_gfx_merge`, `generate_loc_stub`. All return
strings; never write.
